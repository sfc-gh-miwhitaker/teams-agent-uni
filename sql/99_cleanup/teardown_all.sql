 ...
 * 2. SALES_CALLS_ANALYST Cortex Agent (if created)
 * 3. GENERATE_SAFE_JOKE function
 * 4. CORTEX_DEMO schema
 * 5. SNOWFLAKE_EXAMPLE database (if empty or force cascade)
 * 6. SFE_CORTEX_AGENTS_WH warehouse
 * 7. SFE_ENTRA_ID_CORTEX_AGENTS_INTEGRATION security integration
 * 
...
-- Replace schema placeholder if you granted access to a production semantic schema during setup
-- REVOKE USAGE ON SCHEMA <DB>.<SCHEMA> FROM ROLE SALES_CALLS_AGENT_ROLE IGNORE FAILURE;
-- REVOKE SELECT ON VIEW <DB>.<SCHEMA>.VW_CORTEX_ANALYST_SALES_CALL_ACTIVITY FROM ROLE SALES_CALLS_AGENT_ROLE IGNORE FAILURE;
...
-- DROP CORTEX AGENT IF EXISTS SNOWFLAKE_EXAMPLE.CORTEX_DEMO.SALES_CALLS_ANALYST;
...
-- ============================================================================
  ...
  * WHAT WAS DELETED:
  * -----------------
 * - Cortex Agents: JOKE_ASSISTANT, SALES_CALLS_ANALYST (if created)
